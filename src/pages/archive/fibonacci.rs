...
pub fn render(slug:String) -> seed::dom_types::Node<Msg> {
    ...
    if slug_int <= 1 {
        ...
        href_prev = format!("/archive/fibonacci/{}/",&(slug_int-numbers_per_page).to_string());
        prev_link = vec![a!["previous ",numbers_per_page.to_string()," fibonacci numbers", attrs!{At::Class => "link", At::Href => href_prev}]];
    }
    let href_next:String = format!("/archive/fibonacci/{}/",&(slug_int+numbers_per_page).to_string());
